...
func (tlv *TLVDecoder) GetLengthField() *ziface.LengthField {
	...
	return &ziface.LengthField{
		MaxFrameLength:      math.MaxUint16 + 20,
...
func (tlv *TLVDecoder) decode(data []byte) *TLVDecoder {
	...
	tlvData.Tag = array[0]
	//Get L
	tlvData.Length = uint32(binary.BigEndian.Uint16(data[22:24]))
	//Determine the length of V. (确定V的长度)
	tlvData.Value = make([]byte, len(data))

	//Get V
	binary.Read(bytes.NewBuffer(data[:]), binary.BigEndian, tlvData.Value)
