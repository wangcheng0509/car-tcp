...
type RouterSlices struct {
	Apis     map[uint8][]ziface.RouterHandler
...
func NewRouterSlices() *RouterSlices {
	return &RouterSlices{
		Apis:     make(map[uint8][]ziface.RouterHandler, 10),
...
func (r *RouterSlices) AddHandler(msgId uint8, Handlers ...ziface.RouterHandler) {
...
func (r *RouterSlices) GetHandlers(MsgId uint8) ([]ziface.RouterHandler, bool) {
	r.RLock()
	defer r.RUnlock()
...
func (r *RouterSlices) Group(start, end uint8, Handlers ...ziface.RouterHandler) ziface.IGroupRouterSlices {
...
func NewGroup(start, end uint8, router *RouterSlices, Handlers ...ziface.RouterHandler) *GroupRouter {
	g := &GroupRouter{
		start:    start,
		end:      end,
...
func (g *GroupRouter) AddHandler(MsgId uint8, Handlers ...ziface.RouterHandler) {
