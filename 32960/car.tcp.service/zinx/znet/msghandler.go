...
// (切片路由添加)
func (mh *MsgHandle) AddRouterSlices(msgId uint8, handler ...ziface.RouterHandler) ziface.IRouterSlices {
	mh.RouterSlices.AddHandler(msgId, handler...)
	return mh.RouterSlices
}

// Group routes into a group (路由分组)
func (mh *MsgHandle) Group(start, end uint8, Handlers ...ziface.RouterHandler) ziface.IGroupRouterSlices {
