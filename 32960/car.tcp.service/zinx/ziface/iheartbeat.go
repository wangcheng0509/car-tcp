...
type IHeartbeatChecker interface {
	...
	BindRouter(uint8, IRouter)
	BindRouterSlices(uint8, ...RouterHandler)
	Start()
	Stop()
...
type HeartBeatOption struct {
	...
	HeartBeatMsgID   uint8            // User-defined ID for heartbeat detection messages(用户自定义的心跳检测消息ID)
...
const (
	HeartBeatDefaultMsgID uint8 = 255
)
