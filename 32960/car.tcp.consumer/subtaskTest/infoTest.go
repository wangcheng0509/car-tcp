...
func InfoTest() {
	...
	bytes = append(bytes, dateParam.Marshal()...)
	// 整车信息
	bytes = append(bytes, byte(0x01))
	...
	bytes = append(bytes, vehicleParam.Marshal()...)
	// 驱动电机数据
	...
	bytes = append(bytes, drivemotorParam1.Marshal()...)
	...
	bytes = append(bytes, drivemotorParam2.Marshal()...)
	// // 燃料电池数据
	bytes = append(bytes, byte(0x03))
	...
	bytes = append(bytes, fuelcellParam.Marshal()...)
	// // 发动机数据
	// bytes = append(bytes, byte(0x04))
	// engineParam := engine{
	// 	CrankshaftSpeed: 50000,
	// 	FuelConsumption: 20000,
	// 	Status:          0x02,
	// }
	// bytes = append(bytes, engineParam.Marshal()...)
	// // 车辆位置数据
	// bytes = append(bytes, byte(0x05))
	// locationParam := location{
	// 	Latitude:  3664729,
	// 	Longitude: 11708357,
	// 	Status:    32,
	// }
	// bytes = append(bytes, locationParam.Marshal()...)
	// // 极值数据
	// bytes = append(bytes, byte(0x06))
	...
	// }
	// bytes = append(bytes, extremeParam.Marshal()...)

	req.Len = uint16(len(bytes))
	req.Data = bytes
	subtask.RealtimeInfoTest(req)
}
